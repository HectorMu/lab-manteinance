...
controller.GetAll = async (req, res) => {
  try {
    const data = await Computers.List();
    res.json(data);
  } catch (error) {
...
controller.GetOne = async (req, res) => {
  try {
    const data = await Computers.FindOne(req.params.id);
    res.json(data);
  } catch (error) {
    console.log("Error" + error);
    res.json({
...
controller.Save = async (req, res) => {
  try {
    const results = await Computers.Create(req.body);
    console.log(results);
    res.json({
      status: true,
...
controller.Update = async (req, res) => {
  try {
    const results = await Computers.Update(req.body, req.params.id);
    console.log(results);
...
controller.Delete = async (req, res) => {
  try {
    const results = await Computers.Delete(req.params.id);
    console.log(results);
    //Si no exite ninguna fila afectada, significa que ese registro no existe.
    if (results.affectedRows === 0) {
      return res.status(400).json({
